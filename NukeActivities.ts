...
export default definePlugin({
  ...
  start(): void {
    ...
    try {
      if (PresenceStore && typeof PresenceStore.getPresence === "function" && !originalGetPresence) {
        console.log("[NukeActivities] Patching PresenceStore.getPresence...");
        ...
        console.warn("[NukeActivities] PresenceStore.getPresence not found, skipping presence patch");
      }
    } catch (e) {
      console.error("[NukeActivities] Failed to patch PresenceStore:", e);
    }

    // Patch dispatcher so presence updates never populate activities in the first place.
    try {
      if (FluxDispatcher && typeof FluxDispatcher.dispatch === "function" && !originalDispatch) {
        console.log("[NukeActivities] Patching FluxDispatcher.dispatch...");
        ...
        console.warn("[NukeActivities] FluxDispatcher.dispatch not found, skipping dispatcher patch");
      }
    } catch (e) {
      console.error("[NukeActivities] Failed to patch FluxDispatcher:", e);
    }
  },
